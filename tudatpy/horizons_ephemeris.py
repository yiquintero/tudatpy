...
def create_ephemeris_dictionary_from_horizons_table(horizons_table):
    ...
    for i in range(len(horizons_table)):
        ...
        current_time_jd_tdb = Time(
            horizons_table["datetime_jd"][i], format="jd", scale="utc"
        )
        julian_days_since_j2000 = (
            current_time_jd_tdb.jd1 - constants.JULIAN_DAY_ON_J2000
        )
        current_time = (
            julian_days_since_j2000 + current_time_jd_tdb.jd2
...
def jpl_horizons_ephemeris_table(body_identifier, central_body_identifier, epochs):
    ...
    epochs_julian_days = [
        epoch / constants.JULIAN_DAY + constants.JULIAN_DAY_ON_J2000 for epoch in epochs
    ]

    # Query Horizons for the required body and time interval
    obj = Horizons(
        id=body_identifier,
        location="@" + central_body_identifier,
        epochs=epochs_julian_days,
    )
...
def jpl_horizons_tabulated_ephemeris_settings(
    body_identifier, central_body_identifier, epochs
):
